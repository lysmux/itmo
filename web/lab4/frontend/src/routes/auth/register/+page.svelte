<script lang="ts">
	...
	const login = async (event: Event) => {
		...
		axios.post<AuthResponse>('/api/auth/register', {
			username: registerData.username,
			password: registerData.password
		}).then(response => {
	...
	const vkRegister = async (event: Event) => {
		event.preventDefault();
...
<div class="container">
	<h1>Регистрация</h1>
	<p class="has-account">Есть аккаунт? <a href={resolve("/auth/login")}>Войти</a></p>
	...
	<form onsubmit={login}>
		...
		<div class="buttons-block">
			<button class="action" type="submit" disabled={!isValid}>Зарегистрироваться</button>
		</div>
	</form>
	...
	<div class="alternative">
		<button class="social" onclick={vkRegister}>
			<VKIcon/>
			ВКонтакте
		</button>
	</div>
